...
# hame progess Section and matrics k section me dikhane k liye ye class used hoga

def sync_metrics_update(context):
    # agar context ya hasAttributes andar k video me state playing True nii hai to retrun kr dega yahi se
    if not context or not hasattr(context, "state") or not context.state.playing:
        return
# ab hme video processor chahiye ab yaha se hm sare video processor ka method call kr skte hai
    processor = getattr(context, "video_processor", None)

    if not processor:
        return 
    # yaha hme video me exercise type bhi chahiye kaun sa krna chahte hai
    exercise = st.session_state.get("exercise_type")

    if not exercise:
        return
    
    # yaha set kr dnege us exercise ko
    processor.set_exercise(exercise)
    latest_metrics = processor.get_latest_metrics()

    if not latest_metrics:
        return
    # yaha uski value  aa jaegi--reps update hote jaega
...
# services.config.workout_config  se Metric file d lenge
...
# yaha jo metrice wo sare update hote jaenge nii to default value de dega automatically

    reps_per_set = st.session_state.get("reps_per_set", 0)
    target_sets = st.session_state.get("target_sets", 0)

# yaha loops me aayenge reps per set default sabse pahle denge
    if reps is not None and reps_per_set > 0 and target_sets > 0:
        sets_completed = reps // reps_per_set  # yaha double divide use kiya hai
        current_set_reps = reps % reps_per_set  # yah hm modulos used kiya hai
        workout_completed = sets_completed >= target_sets 
        # agar hamne target value jitne acheive kr liye hai to workout complete ho chuka hai
    else:
        sets_completed = 0
        current_set_reps = 0
