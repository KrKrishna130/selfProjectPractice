...
# yaha hm style ka logic de denge

def load_css(file_path):
    if os.path.exists(file_path):  
#agar file open hai to markdown k andar attach ho jaegi agar kuch dynamic change file path se to
...
def inject_local_font(font_path, font_name):
    if not os.path.exists(font_path):
        return
    # yaha hm chhoti images ko text k formate ke convert kr denge
    # yaha base64 m4 encode kr rahe hai
    
    with open(font_path, "rb") as f:
        encoded = base64.b64encode(f.read()).decode()
# otf-open type font
    ext = os.path.splitext(font_path)[1].lstrip(".") # extension
    fmt = {"otf": "opentype"}.get(ext, ext)      # formate 
    mime = {"otf": "font/otf"}.get(ext, f"font/{ext}") # mime-MIME Type(tecxt,image,video etc.)
...
# ye hm function video on krne pr call kra denge
...
# findAndPatch se hme jo video input ko webRTC me  pass kra denge yaha
